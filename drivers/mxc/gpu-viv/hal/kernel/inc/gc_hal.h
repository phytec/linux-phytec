 ...
 *
 * The enum gpu_viv_mutex_lock_class cannot contain more than eight subclasses,
 * because the lockdep framework doesn't support more. So we use a general lock
 * class GPU_VIV_MUTEX_OTHER to handle some locks which are located below
 * "Command->mutexContext" and "Command->commitMutex".
 */
enum gpu_viv_mutex_lock_class {
    /* Default lock class for all unmanaged mutexes
     * mutex: Hardware->powerMutex
     *     file: drivers/mxc/gpu-viv/hal/kernel/arch/gc_hal_kernel_hardware.c
     *         [...]
     *     file: drivers/[...]/kernel/archvg/gc_hal_kernel_hardware_vg.c
     *         [...]
     * mutex: <other locks>
     */
    GPU_VIV_MUTEX_NORMAL,

    /*  mutex: Command->mutexContext in struct gckCOMMAND
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_command.c
     *          gckCOMMAND_Detach
     *          gckCOMMAND_Attach
     *          gckCOMMAND_Commit
     *  mutex: Command->commitMutex in struct gckVGCOMMAND
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_command_vg.c
     *          gckVGCOMMAND_Commit
     *      file: drivers/[...]/kernel/archvg/gc_hal_kernel_hardware_vg.c
    ...
    /*  mutex: Os->signalMutex
     *      file: drivers/mxc/gpu-viv/hal/os/linux/kernel/gc_hal_kernel_os.c
     *          gckOS_DestroySignal
     *          gckOS_Signal
     *  mutex: Mmu->pageTableMutex
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_mmu.c
     *          _SetupDynamicSpace
     *          _AllocatePages
     *          _FreePages
     *          gckMMU_FreePagesEx
     *          _FillFlatMapping
     *  mutex: Kernel->virtualBufferLock
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel.c
     *          gckKERNEL_AllocateVirtualCommandBuffer
     *          gckKERNEL_GetGPUAddress
     *          gckKERNEL_QueryGPUAddress
     *  mutex: Command->queueMutex in struct gckVGCOMMAND
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_command_vg
     *          gckVGCOMMAND_Commit
     *          gcmDECLARE_INTERRUPT_HANDLER
     *          _LockCurrentQueue
     *  mutex: Event->freeEventMutex
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_event.c
     *          gckEVENT_FreeRecord
     *          gckEVENT_AllocateRecord
     */
    GPU_VIV_MUTEX_OTHER,

    /*  mutex: gckEVENT_AddList
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_event.c
     *          gckEVENT_AddList
     *          gckEVENT_Submit
     */
    GPU_VIV_MUTEX_EVENT_LIST,

    /*  mutex: Event->eventQueueMutex
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_event.c
     *          gckEVENT_Notify
     ...
     *          gckEVENT_GetEvent
     */
    GPU_VIV_MUTEX_EVENT_QUEUE,

    /*  mutex: gckKERNEL_FindHandleDatbase's mutex
     *      file: drivers/mxc/gpu-viv/hal/kernel/gc_hal_kernel_video_memory.c
    ...
    /* mutex: (os)->memoryLock
     *     file: drivers/mxc/gpu-viv/hal/os/linux/kernel/gc_hal_kernel_os.c
     *       in define MEMORY_LOCK which is used in
     *          gckOS_MapMemory
     *          gckOS_UnmapMemoryEx
     ...
     *      in define MEMORY_MAP_LOCK which is used in
     *          gckOS_MapUserMemory
     *          gckOS_UnmapUserMemory
     */
    GPU_VIV_MUTEX_OS_MEMORY
};

/*
 * This is a list of call paths through the code. They are backtraces, which
 * are generated by the lockdep framework, of the second/third offending lock,
 * with a reference to the first and second acquired lock.
 *
 * There are more call paths which uses locks, but only these triggered the
 * first error by the lockdep framework.
 *
 *     function name         | acquired mutex         | lock class
 *
 *  1) SyS_ioctl
 *     do_vfs_ioctl
 *     drv_ioctl
 *     gckKERNEL_Dispatch
 *     gckCOMMAND_Detach       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     gckCONTEXT_Destroy
 *     _DestroyContext
 *     gckOS_DestroySignal     Os->signalMutex         GPU_VIV_MUTEX_OTHER
 *
 *  2) gckCOMMAND_Commit       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     gckEVENT_Submit         Event->eventListMutex   GPU_VIV_MUTEX_EVENT_LIST
 *     gckEVENT_GetEvent       Event->eventQueueMutex  GPU_VIV_MUTEX_EVENT_QUEUE
 *
 *  3) gckCOMMAND_Commit                               GPU_VIV_MUTEX_COMMAND
 *                             Command->mutexContext (maybe)
 *     gckCONTEXT_Update
 *     gckEVENT_Signal
 *     gckEVENT_AddList        Event->eventListMutex   GPU_VIV_MUTEX_EVENT_LIST
 *
 *  4) gckCOMMAND_Commit       Command->mutexContext (maybe)
 *     gckCONTEXT_Update
 *     gckEVENT_Signal
 *     gckEVENT_AllocateRecord Event->freeEventMutex   GPU_VIV_MUTEX_EVENT_FREE
 *
 *  5) gckCOMMAND_Attach       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     ...
 *     _AllocatePages          Mmu->pageTableMutex     GPU_VIV_MUTEX_OTHER
 *
 *  6) gckCOMMAND_Attach       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     ...
 *     gckKERNEL_AllocateVirtualCommandBuffer          GPU_VIV_MUTEX_OTHER
 *                             Kernel->virtualBufferLock
 *
 *  7) gckCOMMAND_Attach       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     ...
 *     gckKERNEL_GetGPUAddress Kernel->virtualBufferLock GPU_VIV_MUTEX_OTHER
 *
 *  8) gckCOMMAND_Attach       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     ...
 *     gckOS_Signal            Os->signalMutex         GPU_VIV_MUTEX_OTHER
 *
 *  9) gckHARDWARE_SetPowerManagementState             GPU_VIV_MUTEX_NORMAL
 *                             hardware->powerMutex
 *     ...
 *     gckOS_GetPhysicalAddressProcess                 GPU_VIV_MUTEX_OS_MEMORY
 *                             (os)->memoryLock in define MEMORY_LOCK
 *
 *  10)drv_ioctl
 *     gckKERNEL_Dispatch
 *     gckCOMMAND_Commit       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     gckCONTEXT_Update
 *     gckEVENT_Signal
 *     gckEVENT_AddList
 *     gckEVENT_AllocateRecord Event->freeEventMutex   GPU_VIV_MUTEX_OTHER
 *
 *  11)kernel_init
 ...
 *     gpu_probe
 *     gckGALDEVICE_Start
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_NORMAL
 *                             Hardware->powerMutex
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_COMMAND
 *                             command->commitMutex
 *
 *  12)kernel_init
 *     kernel_init_freeable
 ...
 *     gckGALDEVICE_Start
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_NORMAL
 *                             Hardware->powerMutex (maybe)
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_COMMAND
 *                             command->commitMutex (maybe)
 *     gckVGCOMMAND_Allocate
 *     _AllocateCommandBuffer
 *     gckOS_AllocateNonPagedMemory                    GPU_VIV_MUTEX_OS_MEMORY
 *                             (os)->memoryLock in define MEMORY_LOCK
 *
 *  13)kernel_init
 *     kernel_init_freeable
 *     do_one_initcall__platform_driver_register
 *     driver_register
 *     bus_add_driver
 *     driver_attach
 *     bus_for_each_dev
 *     __driver_attach
 *     driver_probe_deviceplatform_drv_probe
 *     gpu_probe
 *     gckGALDEVICE_Start
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_NORMAL
 *                             Hardware->powerMutex (maybe)
 *     gckVGHARDWARE_SetPowerManagementState           GPU_VIV_MUTEX_COMMAND
 *                             command->commitMutex
 *     _CommandStall  (optimized away)
 *     gckVGCOMMAND_Execute
 *     _LockCurrentQueue       Command->queueMutex     GPU_VIV_MUTEX_OTHER
 *
 *  14)gckVGKERNEL_Dispatch
 *     gckVGCOMMAND_Commit     Command->commitMutex    GPU_VIV_MUTEX_COMMAND
 *     ...
 *     _LockCurrentQueue       Command->queueMutex     GPU_VIV_MUTEX_OTHER
 *
 *  15)gckCOMMAND_Commit       Command->mutexContext   GPU_VIV_MUTEX_COMMAND
 *     ...
 *     gckEVENT_AddList        Event->eventListMutex   GPU_VIV_MUTEX_EVENT_LIST
 *
 *  16)gckCOMMAND_Commit                               GPU_VIV_MUTEX_COMMAND
 *                             Command->mutexContext (maybe)
 *     ...
 *     gckEVENT_Submit         Event->eventListMutex   GPU_VIV_MUTEX_EVENT_LIST
 *
 ...
 *     gckCOMMAND_Commit
 *     gckEVENT_AddList
 *     gckVIDMEM_HANDLE_Dereference                    GPU_VIV_MUTEX_HANDLER
 *                     gckKERNEL_FindHandleDatbase's mutex
 *     gckKERNEL_FreeIntegerId database->mutex         GPU_VIV_MUTEX_DATABASE
 *
 *  18)SyS_ioctl
 *     do_vfs_ioctl
 *     drv_ioctl
 ...
 *     gckKERNEL_AllocateIntegerId                     GPU_VIV_MUTEX_DATABASE
 *                             database->mutex
 */
